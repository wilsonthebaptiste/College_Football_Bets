 ...
 * fetched when first visited, and the two viewer pages are also fetched in
 * the background once the first page is up (`prefetchViewerPages`, called by
 * `RootLayout`), so moving from home to a board to a team never waits on the
 * network for code. The admin pages are never prefetched: viewers never
 * download them, nor the auth library they load (auth/adminAuth.ts).
 *
